...
function preload() {
  partyConnect(
    ...
    "jkv-characterSelectionPocV1" // Use a unique room name
  );
...
function setup() {
  ...
  console.log("My ID (will populate):", me.id);
}

// =============================================================================
// DRAW FUNCTION - Updated with improved info text spacing and HOST label
// =============================================================================
function draw() {
  background(20, 30, 40);
...
function drawGameSetup() {
  if (!me.isReady) {
    ...
    if (partyIsHost()) {
      ...
      let blueFlagChosen = allPlayers.some(p => p.team === 'blue' && p.characterId === 'F' && p.hasCharacter);
      let greenFlagChosen = allPlayers.some(p => p.team === 'green' && p.characterId === 'F' && p.hasCharacter);
    ...
    // --- Client Logic for Setup ---
         ...
         let myTeamFlagChosen = allPlayers.some(p => p.team === me.team && p.characterId === 'F' && p.hasCharacter);
         let otherTeamFlagChosen = allPlayers.some(p => p.team !== me.team && p.characterId === 'F' && p.hasCharacter);
...
function drawCharacterList() {
    ...
    let allPlayers = [me, ...guests];
    let myTeamPlayers = allPlayers.filter(p => p.team === me.team && p.hasCharacter);
